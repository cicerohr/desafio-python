...
from src.Exercism.ghost_gobble_arcade_game.arcade_game import (
    eat_ghost,
    lose,
    score,
    win,
...
class GhostGobbleGameTest(unittest.TestCase):
    ...
    def test_ghost_gets_eaten(self):
        self.assertIs(
            eat_ghost(True, True), True, msg='ghost should get eaten'
        )
    ...
    def test_score_when_eating_power_pellet(self):
        self.assertIs(
            score(True, False), True, msg='score when eating power pellet'
        )

    @pytest.mark.task(taskno=2)
    def test_no_score_when_nothing_eaten(self):
        self.assertIs(
            score(False, False), False, msg='no score when nothing eaten'
        )
    ...
    def test_win_if_all_dots_eaten(self):
        self.assertIs(
            win(True, False, False), True, msg='win if all dots eaten'
        )
    ...
    def test_win_if_all_dots_eaten_and_touching_a_ghost_with_a_power_pellet_active(
        ...
        self.assertIs(
            win(True, True, True),
            True,
            msg='win if all dots eaten and touching a ghost with a power '
            'pellet active',
        )
    ...
    def test_dont_win_if_not_all_dots_eaten(self):
        self.assertIs(
            win(False, True, True),
            False,
            msg="don't win if not all dots eaten and touching a ghost with a "
            'power pellet active',
        )

...
from src.Exercism.black_jack.black_jack import (
    can_double_down,
    can_split_pairs,
    higher_card,
    is_blackjack,
...
class BlackJackTest(unittest.TestCase):
    ...
    def test_value_of_card(self):
        ...
        for variant, (card, value) in enumerate(data, 1):
            with self.subTest(
                f'variation #{variant}', input=card, output=value
            ):
    ...
    def test_is_blackjack(self):
        ...
        for variant, (hand, blackjack) in enumerate(data, 1):
            with self.subTest(
                f'variation #{variant}', input=hand, output=blackjack
            ):
    ...
    def test_can_split_pairs(self):
        ...
        for variant, (hand, split_pairs) in enumerate(data, 1):
            with self.subTest(
                f'variation #{variant}', input=hand, output=split_pairs
            ):
                error_msg = f'Hand {hand} {"can" if split_pairs else "cannot"} be split into pairs.'

                self.assertEqual(
                    can_split_pairs(*hand), split_pairs, msg=error_msg
                )
    ...
    def test_can_double_down(self):
        ...
        for variant, (hand, double_down) in enumerate(data, 1):
            with self.subTest(
                f'variation #{variant}', input=hand, output=double_down
            ):

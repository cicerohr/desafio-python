...
# importing names for tests.
# To that end, we are putting a try/catch around imports and throwing specific
# messages to help students
# decode that they need to create and title their constants and functions in a
# specific way.

try:
    from src.Exercism.guidos_gorgeous_lasagna.lasagna import (
        EXPECTED_BAKE_TIME,
        bake_time_remaining,
...
# Here, we are separating the constant import errors from the function name
# import errors
    ...
    if 'EXPECTED_BAKE_TIME' in item_name:
        ...
        raise ImportError(
            f'\n\nMISSING CONSTANT --> \nWe can not find or import the '
            f'constant {item_name} in your'
...
class LasagnaTest(unittest.TestCase):
    ...
    def test_EXPECTED_BAKE_TIME(self):
        failure_msg = (
            'Expected a constant of EXPECTED_BAKE_TIME with a value of 40.'
        )
    ...
    def test_elapsed_time_in_minutes(self):
        ...
        result_data = [
            prep * 2 + elapsed for prep, elapsed in zip(layer_data, time_data)
        ]
        ...
        for variant, (layers, time, total_time) in enumerate(
            ...
            with self.subTest(
                ...
                self.assertEqual(
                    elapsed_time_in_minutes(layers, time),
                    total_time,
                    msg=failure_msg,
                )
    ...
    def test_docstrings_were_written(self):
        ...
        for variant, function in enumerate(functions, start=1):
            with self.subTest(f'variation #{variant}', function=function):
                failure_msg = (
                    f'Expected a docstring for `{function.__name__}`, but '

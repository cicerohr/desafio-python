...
from src.Exercism.little_sisters_vocab.strings import (
    add_prefix_un,
    adjective_to_verb,
    make_word_groups,
    remove_suffix_ness,
...
class LittleSistersVocabTest(unittest.TestCase):
    ...
    def test_add_prefix_un(self):
        ...
        for variant, word, result in zip(
            number_of_variants, input_data, result_data
        ):
            with self.subTest(
                f'variation #{variant}', word=word, result=result
            ):
                self.assertEqual(
                    add_prefix_un(word),
                    result,
